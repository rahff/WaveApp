...
describe("CalendarPolicies", ()=>{
    ...
    it('should verify if the event is in past', async ()=> {
        const resultCommand = await policies.applySaveCalendarEventPolicies(eventStartInPast.asDto(), dateProvider());
    ...
    it('should verify that the end is before the start', async ()=>{
        const resultCommand = await policies.applySaveCalendarEventPolicies({id: "987", title: "endBefore", start: new Date(2022, 10, 6, 8, 0), end: new Date(2022, 10, 5, 9, 0)}, dateProvider());
    ...
    it('should make sure that the event does not ends during another', async ()=>{
        const resultCommand = await policies.applySaveCalendarEventPolicies(eventEndsAfterStartOfAnotherEvent.asDto(), dateProvider());
    ...
    it('should make sure that the event does not start during another', async ()=>{
        const resultCommand = await policies.applySaveCalendarEventPolicies(eventStartAfterStartOfAnotherEventAndEndAfterEnds.asDto(), dateProvider());
        expect(resultCommand.getPayload()).toEqual("cannot have two event at the same time");
    })

    it('should make sure that the event does not during over another', async ()=>{
        const resultCommand = await policies.applySaveCalendarEventPolicies(eventDuringOverAnother.asDto(), dateProvider());
        expect(resultCommand.getPayload()).toEqual("cannot have two event at the same time");
    })

    it('should make sure that the event does not start ans ends during another', async ()=>{
        const resultCommand = await policies.applySaveCalendarEventPolicies(eventStartAndEndsDuringAnother.asDto(), dateProvider());
        expect(resultCommand.getPayload()).toEqual("cannot have two event at the same time");
    })

    it('should pass the test', async ()=>{
        const resultCommand = await policies.applySaveCalendarEventPolicies(correctEvent.asDto(), dateProvider());
